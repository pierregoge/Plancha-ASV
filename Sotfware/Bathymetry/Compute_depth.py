...
OFFSET_POSITION=[0,0] # Offset from GPS to Single Beam, in format [X,Y] with X facing forward and Y facing left
Z_GPS=0.1
Z_SONDEUR=-0.3
DEPTH_COEF=1
...
with open(LOG_PATH) as file:
    ...
    for line in parsed:
        if line[0]=='DPTH':
            ...
            if float(line[2])>=5 and float(line[6])<=2:  #Checking status >= 5 and HDOP <= 2
...
for gps2 in gps2_list:
    ...
    if abs(attitude_centered[0])<ANGLE_MAX and abs(attitude_centered[1])<ANGLE_MAX: #Checking the angle is valid
        ...
        if depth !=-1 : #Keeping only valid points
            ...
            prof_ign=hauteur+rot_vect_gps2pt[2]-alt_geoid
